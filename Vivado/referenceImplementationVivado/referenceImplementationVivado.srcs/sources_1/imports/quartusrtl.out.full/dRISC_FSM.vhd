...
variable Ai: STD_LOGIC_VECTOR (MEMORY_WIDTH-1 DOWNTO 0) := (others => '0');
variable Bi: STD_LOGIC_VECTOR (MEMORY_WIDTH-1 DOWNTO 0) := (others => '0');
variable Ci: STD_LOGIC_VECTOR (MEMORY_WIDTH-1 DOWNTO 0) := (others => '0');
variable Di: STD_LOGIC_VECTOR (MEMORY_WIDTH-1 DOWNTO 0) := (others => '0');
variable Ei: STD_LOGIC_VECTOR (MEMORY_WIDTH-1 DOWNTO 0) := (others => '0');
variable Fi: STD_LOGIC_VECTOR (MEMORY_WIDTH-1 DOWNTO 0) := (others => '0');
variable MCRi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable CHRi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable IWRi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable ICRi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable CSRi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable ISRi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable IDRi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable IORi: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');
variable ISRi_new: std_logic_vector(REG_WIDTH-1 downto 0) := (others => '0');	
variable ALU_DATAi: signed(MEMORY_WIDTH-1 DOWNTO 0) := (others => '0');
	...
	if RST = '1' then
		...
		if CPU_STOPin = '1' then
			case (STATE) is
				...
				when RESET => 	
							-- Reset State will be handeled by Initital mealy in clash
							PCi := 0;
							PCi_new :=0;
							PCi_newp :=0;
				...
				when CPU_CYCLE_START => 		
							...
							-- internal IORi variable updated from ioctrl
							IORi := IORin;
							
							-- Memaddress for IOR_Register 
							MEM_address <= IOR_addr;

							-- MEM_REG_TAP is the BitMap for the In and Out part of the IOR_Register ??
							-- & ist CONCAT !!!
							MEM_data <= MEM_REG_TAP & std_logic_vector(IORi);
							-- Write The BitMapped Registers to Memory
							MEM_wren <= '1';
							MEM_rden <= '0';
							
							-- F_Mode 0 is One Instuction Mode
							-- Bootstrap Regs is the count of registers to be simply loaded from RAM before execution starts
							...
							-- Output PC for ALU -- we wont have to do this because we directly call the ALU 
							PC <= std_logic_vector(to_unsigned(PCi, MEMORY_WIDTH));
				...
				when FETCH_MCR =>	  				
							-- Request MCR from Memory
							MEM_address <= std_logic_vector(to_unsigned(PCi, MEMORY_WIDTH));
							MEM_rden <= '1';
							MEM_wren <= '0';		
					
							-- Why Not FETCH_A_SAVE_MCR_STATE ??
							NEXT_STATE <= FETCH_0_SAVE_MCR;			
							
				when FETCH_0_SAVE_MCR =>	 							
							-- Save MCR in internal Register
							MCRi := MEM_q(REG_WIDTH-1 downto 0);
													
							MEM_rden <= '0';		
							MEM_wren <= '0';							
							-- Again Why Not combine FETCH_A_SAVE_MCR_STATE ??
							NEXT_STATE <= FETCH_A;
							
							-- Why do we have MCRu and MCRi ??
							MCRu <= MCRi;		
				...
				when FETCH_0_SAVE_F =>		
							...
							if PCi < BOOTSTRAP_REGS then
								-- Still loading Bootsrap Registers
				...
				when BOOTSTRAP_CTRL =>	
							-- This will only use the memory in A
							-- Why Not only load A before jumping into EXEC_CTRL ??
							PCi_new := PCi + 1;
							...
							if PCi = to_integer(unsigned(MCR_addr)) then
								-- Set Execution Mode (MultiInstruction or SingleInstruction)
								-- Depending on the value of the MCR
								MCRi := Ai(REG_WIDTH-1 downto 0);

								-- What is the difference between MCRu and MCRi?
								MCRu <= MCRi;
								...
								-- Update Interrupt Configuration Register
								ICRi := Ai(REG_WIDTH-1 downto 0);
								ICR <= ICRi;
							elsif PCi = to_integer(unsigned(CSR_addr)) then
								-- Update Clock Speed Register
								-- We will have to see how to implement different Clock Speeds in Clash
				...
				when EXEC_CTRL => 
							-- I think this State is waiting for ALU Execution to finish
							-- Else it looks like a wasted Clock Cycle
							MEM_rden <= '0';
							MEM_wren <= '0';		
				...
				when LEQ_PC_WRITE_BACK => 
							...
							-- Update next PC according to SUBLEQ Rules
							-- negative -> take c as new pc else add Offset
							...
							-- This looks ugly we will take different conditionals
							if MCRi = MCR_pc(REG_WIDTH-1 downto 0) then 
								NEXT_STATE <= CPU_CYCLE_START;	
				...
				when TTA_MEM_WRITE_BACK =>
							-- Still dont know what TTA means
							MEM_rden <= '0';
							MEM_wren <= '0';		
